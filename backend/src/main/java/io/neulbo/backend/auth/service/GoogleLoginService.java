...
public class GoogleLoginService extends AbstractOAuthLoginService {
    ...
    private final OAuth2Properties.Registration.ClientConfig googleClient;
    private final OAuth2Properties.Provider.ProviderConfig googleProvider;

    public GoogleLoginService(WebClient webClient, UserRepository userRepository, JwtProvider jwtProvider, OAuth2Properties oAuth2Properties) {
        super(webClient, userRepository, jwtProvider);
        this.googleClient = oAuth2Properties.registration().google();
        this.googleProvider = oAuth2Properties.provider().google();
    }
    ...
    public Mono<OAuthToken> getTokenReactive(String code) {
        ...
        return webClient.post()
                ...
                .body(BodyInserters.fromFormData("grant_type", "authorization_code")
                        .with("client_id", googleClient.clientId())
                        .with("client_secret", googleClient.clientSecret())
                        .with("redirect_uri", googleClient.redirectUri())
                        .with("code", code))
                .retrieve()
                .onStatus(
